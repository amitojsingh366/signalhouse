...
- [x] Cash-aware actions + iOS tab reorganization — `actionable` field on BUY actions (false if insufficient cash/slots), separate "Signals" section on web + iOS, actionPlan auto-refreshes on trade create/edit/delete, cash, holding, and upload changes, Trades promoted to tab 3, Pre-Market to More
...
- [x] Treat manual portfolio edits as capital adjustments, not PnL — `total_pnl = realized_pnl_from_sells + unrealized_pnl_on_open_positions`, while cash edits, holding corrections, trade replay, and upload syncs shift snapshots/capital baselines instead of creating fake gains. Daily PnL compares against the latest prior ET market-day snapshot so uploads and UTC midnight do not distort the daily number.
...
- [x] Trade edit/delete with ledger replay — added `PUT` / `DELETE /api/trades/{id}`, web edit/delete controls, and iOS tap/swipe editing. Mutations replay the trade ledger to recalculate holdings, cash, sell P&L, risk-manager open positions, snapshots, and action-plan/recommendation caches.
