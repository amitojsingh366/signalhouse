...
- [x] Signal snooze + clickable action charts — Snooze sell/stop-loss signals for 4h (configurable), auto-unsnooze if loss worsens by 3%+ from snooze point. DB: `signal_snoozes` table (symbol, expires_at, pnl_pct_at_snooze). API: `POST /api/signals/snooze`, `DELETE /api/signals/snooze/{symbol}`, `GET /api/signals/snoozed`. Web: click any action card to expand inline price chart, snooze/unsnooze buttons, collapsible snoozed section. iOS: NavigationLink from action rows to ActionDetailView with price chart, swipe-to-snooze/unsnooze, DisclosureGroup for snoozed.
