...
from trader.bot import TraderBot, is_market_hours
from trader.cogs.signals import RecheckView, _build_buy_embed, _build_sell_embed
...
class TasksCog(commands.Cog):
    ...
    async def scan_loop(self) -> None:
        ...
        try:
            ...
            held_symbols = list(self.bot.portfolio.holdings.keys())
            if held_symbols:
                prices = await self.bot.market_data.get_batch_prices(held_symbols)
                alerts = await self.bot.strategy.get_exit_alerts(prices)
                for alert in alerts:
                    embed = discord.Embed(
            ...
            recs = await self.bot.strategy.get_top_recommendations(n=3)
    ...
    async def daily_status_loop(self) -> None:
        ...
        try:
            ...
            self.bot.portfolio.record_daily_snapshot(prices)

            pnl_data = self.bot.portfolio.get_daily_pnl(prices)
            holdings = self.bot.portfolio.get_holdings_with_pnl(prices)
    ...
    async def premarket_loop(self) -> None:
        ...
        try:
            movers = await self.bot.strategy.get_premarket_movers()
            if not movers:
                return
    ...
    async def morning_briefing_loop(self) -> None:
        ...
        try:
            insights = await self.bot.strategy.get_daily_insights()
