...
class TradingCog(commands.Cog):
    ...
    async def buy(
        ...
        trade = await self.bot.portfolio.record_buy(symbol, quantity, price, self.bot.risk)
        ...
        embed.add_field(
            name="Cash Remaining", value=f"${self.bot.portfolio.cash:.2f}", inline=False
        )
    ...
    async def sell(
        ...
        trade = await self.bot.portfolio.record_sell(symbol, quantity, price, self.bot.risk)
