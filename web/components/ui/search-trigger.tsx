"use client";

import { Search } from "lucide-react";

/** Inline button that opens the Cmd+K command search modal */
export function SearchTrigger({ className }: { className?: string }) {
  function open() {
    // Dispatch Cmd+K to trigger the CommandSearch modal
    document.dispatchEvent(
      new KeyboardEvent("keydown", { key: "k", metaKey: true, bubbles: true })
    );
  }

  return (
    <button
      onClick={open}
      className={`flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-400 transition-colors hover:bg-white/10 hover:text-slate-300 ${className ?? ""}`}
    >
      <Search className="h-4 w-4" />
      <span className="hidden sm:inline">Search symbol</span>
      <kbd className="hidden rounded border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] text-slate-500 sm:inline-block">
        ⌘K
      </kbd>
    </button>
  );
}
